...
public class Array<T> implements Iterable<T> {
	...
	public boolean contains(T value) {
		for (int i = 0; i < this.length; i++) {
			if (value.equals(this.array[i])) return true;
		}
		return false;
	}
    ...
    public void remove(T value) {
		...
		for (int i = 0; i < n; i++) {
			if (i != j) this.array[j] = this.array[i];
			if ( ! value.equals(this.array[i])) j++;
		}
	...
	public boolean equals(Iterable other) {
		...
		for (int i = 0; i < this.length; i++) {
			...
			if ( ! iterator.next().equals(this.array[i])) return false;
		}
